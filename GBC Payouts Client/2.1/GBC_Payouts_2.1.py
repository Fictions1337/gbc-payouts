# +----------------------------------------------+
# | Gallywix Boosting Community Payout Tool v2.1 |
# +----------------------------------------------+

# <-------------------IMPORTS START------------------->
# Regex for parsing the header for file name
import re
# Psutil and OS to terminate the old client
import psutil, os
# Subprocess to open the new client
import subprocess
...
init()
# Self Auto Update System
current_version = 2.1
response = requests.get('http://159.69.154.56:9006/client/update')

new_version = float(response.text)

def filename_from_header(cd):
    if not cd:
        return None
...
if new_version > current_version:
    print("Downloading updates. Please wait...")
    download = requests.get('http://159.69.154.56:9006/client/download')
    file_name = filename_from_header(download.headers.get('Content-Disposition'))
    with open(file_name, 'wb') as f:
        f.write(download.content)
    subprocess.Popen([file_name], close_fds=True)
    time.sleep(0.1)
    for process in (process for process in psutil.process_iter() if process.name()=="GBC_Payouts_" + str(current_version) + ".exe"):
        process.kill()
old_version = current_version - 0.1
if os.path.exists("GBC_Payouts_" + str(old_version) + ".exe"):
    for process in (process for process in psutil.process_iter() if process.name()=="GBC_Payouts_" + str(old_version) + ".exe"):
        process.kill()
    os.remove("GBC_Payouts_" + str(old_version) + ".exe")
    print("Sucessfully updated to version " + str(current_version))
